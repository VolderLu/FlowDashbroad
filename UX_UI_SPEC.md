...
#### 專注選擇互動（v2.2 新增）
- IDLE 狀態下，點擊項目（非 checkbox/input）→ 預選該項目
- 再次點擊已選中項目 → 取消預選
- 預選項目顯示 `.top3__item--selected` 樣式
- FOCUS 時，非選中項目隱藏（`display: none`）
...
| 預選中（IDLE） | primary 邊框 + 微暖背景 + 光暈邊框 | 如選中的茶具 |
